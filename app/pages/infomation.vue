<template>
  ...
  >
    <NuxtImg
      src="/photo/wallpaper/91fd2fe5-b82b-4f16-b65f-76f124383c12.jpg"
      alt="服务器背景墙纸"
      class="absolute inset-0 w-full h-full object-cover select-none pointer-events-none"
      preload
    />
    <div
      class="absolute inset-0 bg-gradient-to-t from-black/95 via-black/80 to-black/60 backdrop-blur-[2px]"
    ></div>

    <div
      class="relative z-10 flex flex-col items-center justify-center max-w-5xl w-full gap-5 px-2 md:px-6"
    >
      <!-- Header -->
      <header
      ...
      >
        ...
        <div class="flex items-center gap-3">
          <div
            class="bg-black/50 border border-white/10 px-4 py-2.5 rounded-xl flex items-center gap-3"
          >
      ...
      <main class="flex flex-col gap-4 w-full">
        ...
        >
          ...
          >
            <div class="flex items-center gap-3 min-w-0">
              ...
              <span
                class="text-sm font-mono text-emerald-300 font-bold select-all truncate"
              >
                {{ fullServerAddress }}
              </span>
            </div>
          ...
          <div
            class="flex items-center gap-2.5 w-full md:w-auto justify-between md:justify-end text-xs flex-wrap"
          >
            <div
              class="bg-black/30 border border-white/5 px-3.5 py-2 rounded-xl flex items-center gap-2"
            >
            ...
            >
              ...
              >
              <span
                class="font-bold font-mono text-gray-200 text-xs truncate max-w-[120px]"
                :title="softwareName"
              >
        ...
        >
          ...
          >
            <h2
              class="text-xs md:text-sm font-bold uppercase tracking-wider text-gray-200 flex items-center gap-2"
            >
          ...
          <div class="flex-1 overflow-y-auto max-h-[320px] pr-1 custom-scrollbar">
            <!-- Loading -->
            <template
              v-if="displayState === SERVER_STATE.LOADING && !hasCachedPlayers"
            >
            ...
            <template v-else-if="playersList.length > 0">
              <div
                class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 p-0.5"
              >
                <div
                ...
                >
                  ...
                  <span
                    v-if="isStaff(getPlayerName(player))"
            ...
            <template v-else>
              <div
                class="h-full flex flex-col items-center justify-center text-center text-gray-400 py-10"
              >
                <i
                  class="fa-solid fa-ghost text-4xl mb-3 text-gray-600 animate-pulse"
                ></i>
...
/* ===================== 数据拉取 ===================== */
/**
 * 优先从 siteConfig 读取 SERVER_STATUS API 模板
 */
const fetchAllServers = async () => {
  loading.value = true;
  const statusApiTpl =
    siteConfig.API_ENDPOINTS?.SERVER_STATUS ||
    "https://api.hanximeng.com/mc/?server_addr={ip}&server_port={port}";
