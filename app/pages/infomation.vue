<template>
            ...
            <div class="flex flex-col items-center min-[928px]:items-start gap-3">
              ...
              <div
                ...
                <div class="text-left flex-1">
                  ...
                  <p class="text-xs font-mono text-gray-200 mt-0.5 select-all">{{ serverIp }}:{{ serverPort }}</p>
                </div>
                <button @click="copyIp"
          ...
          <div
            ...
            <div class="flex items-center gap-3">
              <div class="relative flex h-2.5 w-2.5">
                ...
                <span
                  :class="loading ? 'bg-amber-500' : (isOnline ? 'bg-emerald-500' : 'bg-rose-500')"
              ...
              <div class="text-left">
                ...
                <p class="text-xs font-bold mt-0.5 text-gray-200">
                  {{ loading ? '正在检测...' : (isOnline ? '在线 / 运行中' : '离线 / 维护中') }}
                </p>
              </div>
            </div>
        ...
        <section class="grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
          ...
          <div
            ...
            <div class="text-left">
              ...
              <p class="text-base font-black mt-0.5 text-gray-100">
                {{ loading ? '--' : (isOnline ? `${onlinePlayersCount} / ${maxPlayersCount}` : '0 / 0') }}
              </p>
            </div>
          </div>
          ...
          <div
            ...
            <div class="text-left max-w-[calc(100%-3.5rem)]">
              ...
              <p class="text-base font-black mt-0.5 text-gray-100 truncate" :title="softwareName">
                {{ loading ? '--' : (isOnline ? softwareName : '无') }}
              </p>
            </div>
          </div>
          ...
          <div
            ...
            <div class="text-left max-w-[calc(100%-3.5rem)]">
              ...
              <p class="text-base font-black mt-0.5 text-gray-100 truncate" :title="versionName">
                {{ loading ? '--' : (isOnline ? versionName : '无') }}
              </p>
            </div>
          </div>
        </section>
        ...
        <section
          ...
          <div
            ...
            <h2 class="text-xs font-bold uppercase tracking-wider text-gray-300 flex items-center gap-2">
              ...
              <span>实时在线玩家 ({{ onlinePlayersCount }})</span>
            </h2>
            <div
          ...
          <div class="flex-1 overflow-y-auto max-h-[340px] pr-1 custom-scrollbar">
            ...
            <!-- 玩家渲染网格 -->
            <template v-else-if="isOnline && playersList.length > 0">
              <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 p-0.5">
                <div v-for="(player, index) in playersList" :key="getPlayerUuid(player) || getPlayerName(player)"
                  ...
                  <div class="flex items-center gap-3 min-w-0">
                    <div
                      class="shrink-0 w-8 h-8 rounded-lg overflow-hidden border border-white/10 flex items-center justify-center relative bg-black/50">
            ...
            <template v-else>
              <div class="h-full flex flex-col items-center justify-center text-center text-gray-500 py-12">
                ...
                <p class="text-xs tracking-wide">
                  {{ isOnline ? '四周寂静无声，暂无在线玩家' : '服务器可能正在打盹（离线中）' }}
                </p>
              </div>
            </template>
          </div>
  ...
  const maxPlayersCount = computed(() => {
    return infoData.value?.players?.max ?? 0
  })
  
  // 实时在线玩家列表（匹配 sample 数组）
  const playersList = computed(() => {
    return infoData.value?.players?.sample || infoData.value?.players?.list || []
  })
  
  // 服务端架构与版本 (如 "Paper 1.21.10")
  const softwareName = computed(() => {
    const fullName = infoData.value?.version?.name || ''
    if (fullName.includes(' ')) {
      return fullName.split(' ')[0] // 提取出 Paper
    }
    return fullName || 'Minecraft'
  })
  
  const versionName = computed(() => {
    const fullName = infoData.value?.version?.name || ''
    if (fullName.includes(' ')) {
      return fullName.split(' ').slice(1).join(' ') // 提取出 1.21.10
    }
    return fullName || '1.21.10'
  })
  ...
  // 获取玩家 UUID (如 c4afcf6b-0039-384d-b397-fe839872b536 -> 清除短横杠)
  const getPlayerUuid = (player) => {
    if (typeof player === 'object' && player?.id) {
      return player.id.replace(/-/g, '')
    }
    if (typeof player === 'object' && player?.uuid) {
      return player.uuid.replace(/-/g, '')
    }
    return null
  }
  ...
  const getAvatarUrl = (player) => {
    ...
    if (uuid) {
      return `https://api.mineatar.io/face/${uuid}?size=32&default=MHF_Steve`
    }
  ...
  const fetchServerStatus = async () => {
    loading.value = true
    try {
      const targetUrl = `https://api.hanximeng.com/mc/?server_addr=${serverIp}&server_port=${serverPort}`
      ...
      console.error('获取 MC 服务器状态失败:', err)
  ...
  onMounted(() => {
    fetchServerStatus()
    timer = setInterval(fetchServerStatus, 60000)
  })
